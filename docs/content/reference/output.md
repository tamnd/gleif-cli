...
describes what users get. Pick a format with `-o`, or let gleif choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gleif <command> -o table   # aligned columns for reading
gleif <command> -o jsonl   # one JSON object per line, for piping
gleif <command> -o json    # a single JSON array
gleif <command> -o csv     # spreadsheet friendly
gleif <command> -o tsv     # tab-separated
gleif <command> -o url     # just the URL column
gleif <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gleif <command> --fields id,title,url
```
...
```bash
gleif <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gleif <command>            # a table, because this is a terminal
gleif <command> | wc -l    # JSONL, because this is a pipe
```
